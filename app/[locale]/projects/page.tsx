...
import ProjectsFilters, {
  type ProjectItem,
} from "./components/ProjectsFilters";
...
export default async function Projects({ params }: ProjectsPageProps) {
  ...
  const types: Project["type"][] = ["personal", "professional", "academic"];
  const typeLabels = Object.fromEntries(
    types.map((type) => [type, t(`types.${type}.label`)]),
  );
  ...
  return (
    <div className={styles.page}>
      ...
      <ProjectsFilters items={items} typeLabels={typeLabels} />
    </div>
  );
}
